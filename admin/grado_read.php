...
if(!empty($_POST))
{
	...
	$sql = "SELECT * FROM años WHERE año LIKE ? ORDER BY año LIMIT $page1,20";
	$params = array("%$search%");
}
else
{
	$sql = "SELECT * FROM años ORDER BY año LIMIT $page1,20";
	$params = null;
}
...
if($data != null)
{
	$tabla = 	"<table class='centered striped'>
		...
		{
	        $tabla .=	"<tr>
	            			<td>".htmlspecialchars($row['id_año'])."</td>
	            			<td>".htmlspecialchars($row['año'])."</td>
	            			<td>
	            				<a href='grado_save.php?id=".base64_encode(htmlspecialchars($row['id_año']))."' class='btn  yellow lighten-2'><i class='material-icons'>edit</i></a>
								<a href='grado_delete.php?id=".base64_encode(htmlspecialchars($row['id_año']))."' class='btn red'><i class='material-icons'>delete</i></a>
							</td>
	        			</tr>";
		}
		$tabla .= 	"</tbody>
...
<ul class="pagination center-align">
	<?php
	$cons2 = ("SELECT COUNT(id_año) FROM años");
            $parametros = null;
            ...
            //Si la paginacion es 1, el 1 estara deshabilitado
                ...
                //caso contrario esta habilitado
                    ...
                    $page2 = $page - 1;
                    $pagi = "<li class='waves-efect'><a href='grado_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_left</i></a></li>";
                }
                ...
                for($i = 1; $i<=$filas; $i++) {
                    if($page==$i)
                    {
                     $pagi .= "<li class='active blue'><a href='grado_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                    //Si no solo se imprimira
                    else{
                        $pagi .= "<li class='waves-effect'><a href='grado_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                }
                ...
                if($page == $filas)
                {
                    ...
                    $page2 = $page + 1;
                    $pagi .= "<li class='waves-efect'><a href='grado_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_right</i></a></li>";
                }
                print($pagi);
	?>
</ul>
