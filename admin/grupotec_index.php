...
<html lang="es">
	...
	<body>
		...
		if(!empty($_POST))
		{
			...
			$sql = "SELECT * FROM grupo_tecnico WHERE grupo_tec LIKE ? ORDER BY id_grupo_tec LIMIT $page1,20";
			$params = array("%$search%");
		}
		else
		{
			$sql = "SELECT * FROM grupo_tecnico ORDER BY id_grupo_tec LIMIT $page1,20";
			$params = null;
		}
		...
		if($data != null)
		{
			$tabla = 	"<div class='card-panel paneles'>
				...
				{
					$tabla .= 	"<tr>
									<td>".htmlspecialchars($row['id_grupo_tec'])."</td>
									<td>".htmlspecialchars($row['grupo_tec'])."</td>
									<td>
										<a href='grupotec_save.php?id=".base64_encode(htmlspecialchars($row['id_grupo_tec']))."' class='btn blue'><i class='material-icons'>edit</i></a>
	...
	<?php
	$cons2 = ("SELECT COUNT(id_grupo_tec) FROM grupo_tecnico");
            $parametros = null;
            ...
            //Si la paginacion es 1, el 1 estara deshabilitado
                ...
                //caso contrario esta habilitado
                    ...
                    $page2 = $page - 1;
                    $pagi = "<li class='waves-efect'><a href='grupotec_index.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_left</i></a></li>";
                }
                ...
                for($i = 1; $i<=$filas; $i++) {
                    if($page==$i)
                    {
                     $pagi .= "<li class='active blue'><a href='grupotec_index.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                    //Si no solo se imprimira
                    else{
                        $pagi .= "<li class='waves-effect'><a href='grupotec_index.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                }
                ...
                if($page == $filas)
                {
                    ...
                    $page2 = $page + 1;
                    $pagi .= "<li class='waves-efect'><a href='grupotec_index.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_right</i></a></li>";
                }
                print($pagi);
	?>
	</ul>
