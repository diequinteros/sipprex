	...
	$tabla = 	"<table class='centered striped'>
		...
		foreach($data as $row)
		{
	        $tabla .=	"<tr>
	            			...
	            			<td>
	            				<a href='seccion_save.php?id=".base64_encode(htmlspecialchars($row['id_seccion']))."' class='btn  yellow lighten-2'><i class='material-icons'>edit</i></a>
	...
	$cons2 = ("SELECT COUNT(id_seccion) FROM secciones");
            ...
            //Si la paginacion es 1, el 1 estara deshabilitado
                ...
                //caso contrario esta habilitado
                    ...
                    $page2 = $page - 1;
                    $pagi = "<li class='waves-efect'><a href='seccion_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_left</i></a></li>";
                }
                ...
                for($i = 1; $i<=$filas; $i++) {
                    if($page==$i)
                    {
                     $pagi .= "<li class='active blue'><a href='seccion_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                    //Si no solo se imprimira
                    else{
                        $pagi .= "<li class='waves-effect'><a href='seccion_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                }
                ...
                if($page == $filas)
                {
                    ...
                    $page2 = $page + 1;
                    $pagi .= "<li class='waves-efect'><a href='seccion_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_right</i></a></li>";
                }
                print($pagi);
	?>
	</ul>
