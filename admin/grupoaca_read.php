...
if(!empty($_POST))
{
	...
	$sql = "SELECT * FROM grupo_academico WHERE grupo_aca LIKE ? ORDER BY grupo_aca LIMIT $page1,20";
	$params = array("%$search%");
}
else
{
	$sql = "SELECT * FROM grupo_academico ORDER BY grupo_aca LIMIT $page1,20";
	$params = null;
}
	...
	$tabla = 	"<table class='centered striped'>
		...
		foreach($data as $row)
		{
	        $tabla .=	"<tr>
	            			...
	            			<td>
	            				<a href='grupoaca_save.php?id=".base64_encode(htmlspecialchars($row['id_grupo_aca']))."' class='btn  yellow lighten-2'><i class='material-icons'>edit</i></a>
								<a href='grupoaca_delete.php?id=".base64_encode(htmlspecialchars($row['id_grupo_aca']))."' class='btn red'><i class='material-icons'>delete</i></a>
							</td>
	        			</tr>";
		}
		$tabla .= 	"</tbody>
...
//Se cuentan los productos
            $cons2 = ("SELECT COUNT(id_grupo_aca) FROM grupo_academico");
            $parametros = null;
            ...
            //Si la paginacion es 1, el 1 estara deshabilitado
                ...
                //caso contrario esta habilitado
                    ...
                    $page2 = $page - 1;
                    $pagi = "<li class='waves-efect'><a href='grupoaca_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_left</i></a></li>";
                }
                ...
                for($i = 1; $i<=$filas; $i++) {
                    if($page==$i)
                    {
                     $pagi .= "<li class='active blue'><a href='grupoaca_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                    //Si no solo se imprimira
                    else{
                        $pagi .= "<li class='waves-effect'><a href='grupoaca_read.php?page=".base64_encode($i)."'>$i</a></li>";
                    }
                }
                ...
                if($page == $filas)
                {
                    ...
                    $page2 = $page + 1;
                    $pagi .= "<li class='waves-efect'><a href='grupoaca_read.php?page=".base64_encode($page2)."'><i class='material-icons'>chevron_right</i></a></li>";
                }
                print($pagi);
